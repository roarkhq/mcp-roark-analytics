...
name: roark-concepts
description: >-
  Canonical definitions of the Roark domain objects: personas, customer flows,
  scripted flows, run plans, templates and metrics. Read a concept when you need
  to know what a Roark object IS, which fields it has, or how it relates to the
  others, rather than how to call the API for it. The other Roark skills
  reference these files instead of restating them.
---
...
Shared with the in-product assistant, so the two cannot describe the product
differently. Read the concept you need; do not read all of them up front.
...
A concept says what a thing is. The workflow and the SDK calls belong to the
skill that referenced it.
